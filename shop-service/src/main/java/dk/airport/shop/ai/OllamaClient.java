 ...
 *
 * <p>Every call runs in JSON mode ({@code "format": "json"}: the server constrains decoding to valid JSON),
 * without streaming and with {@code temperature 0}, so the same prompt gives the same answer. The client is built
...
public class OllamaClient {
    ...
    /**
     * One chat completion in JSON mode. Returns the assistant's message content, which is a JSON document.
     *
